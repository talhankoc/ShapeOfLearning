...
function ff2boundary(farfaces,firstv;setcard=1)
	rv = Array{Int64}(0)
	cp = [1]
	if setcard == 1
		rv = Array{Int64}(0)
		...
		n = length(farfaces[setcard])		
		rv = ff2aflight(farfaces,firstv,setcard,1:n)
		rv = vec(rv)
		cp = convert(Array{Int64,1},setcard+1:setcard:(1+n*setcard))		
		prepend!(cp,[1])	
	end
	return rv,cp
end
...
function ff2complex(farfaces,firstv;maxcard = length(farfaces))
	...
	for setcard = 2:maxcard
		Nrv[setcard],Ncp[setcard] = ff2boundary(farfaces,firstv,setcard=setcard)
	end
	return Nrv,Ncp
end

function ocff2of(filtration::Array{Int64},filtrationtranslator::Array{Int64})
	m = length(filtration)	
	filt = Array{Int64}(m)
	for i = 1:m
		filt[i] = filtrationtranslator[filtration[i]]
	end
	return filt
end
...
function filteredmatrixfromfarfaces{Tv}(
	...
	verbose = false)	
	### Comments
	### The low filtration is to be arranged least to greatest	
	...
	### that the entries of lowfiltration[nplows[perm]] appear in ascending order, numer-
	### ically.  
	### ======= 
	### lowtranslator
	### The length-numlows vector with the maximum possible number of zeros subject to
	### the condition that lowtranslator[lowlab]=1:length(lowlab).
	### ======= 
	### Mrv, Mcp, Mm
	### Sparse matrix representation of transpose(D[lowlab,higlab]), where D is submatrix 
	### of the total boundary operator indexed by cells of dimension setcard-1 (along the
	### columns) and setcard-2 (along the rows).
	### ======= 	
...
function filtration2maxcard(filtration)
	c = 0
	for i = 1:length(filtration)
		if !isempty(filtration[i])
			c = i
		end
	end
	return c
...
function cellcount(C)
	c = 0
	for i = 1:length(C["filtration"])
		c+= length(C["filtration"][i])
	end
	return c
end

function ocff2of(filtration::Array{Int64},filtrationtranslator::Array{Float64})
	m = length(filtration)	
...
function ocff2of(filtration::Array{Array{Int64,1},1},filtrationtranslator::Array{Float64})
	n = length(filtration)
	filt = Array{Array{Float64}}(n)
	for i = 1:n
		filt[i] = ocff2of(filtration[i],filtrationtranslator)
	end
	return filt
end

function floatfiltration(C)
	filtration = convert(Array{Array{Int64}},C["filtration"])
	filtrationtranslator = C["filtrationtranslator"]
	return ocff2of(filtration,filtrationtranslator)
end
...
function morseLU{Tv<:Integer}(
	...
	#### lowlab0 & lowtranslator0 seem to be used only for diagnostic purposes
...
function persistF2_core_cell(
	...
	###
	###
	### I believe that lowtranslator and lowlab0 are vestigal and may be removed
	###
	###
	...
	trv 			= fill(Array{Int64}(0),maxcard+1)
	tcp 			= fill(Array{Int64}(0),maxcard+1)
	phigs			= fill(Array{Int64}(0),maxcard+1)
	plows			= fill(Array{Int64}(0),maxcard+1)
	lowtranslator 	= fill(Array{Int64}(0),maxcard+1)
	higtranslator	= fill(Array{Int64}(0),maxcard+1)
	lowlab0			= fill(Array{Int64}(0),maxcard+1)
	tid				= fill(Array{Int64}(0),maxcard+1)
	m 				= length(Ncp[1])-1
	tcp[1]			= fill(1,m);
	tcp[maxcard+1]  = [1];		
	initialnnz 		= 0	
	for setcard = 2:maxcard
		if setcard > length(Nrv)
			trv[setcard] = Array{Int64}(0)
			tcp[setcard] = ones(Int64,1)
			tid[setcard] = Array{Int64}(0)
			plows[setcard] = Array{Int64}(0)
			phigs[setcard] = Array{Int64}(0)
			continue
		elseif setcard>2
		...
		Mrv 			= Nrv[setcard] # temporary
		Mcp 			= Ncp[setcard] # temporary
		if isempty(Mrv)
			trv[setcard] 	= Array{Int64}(0)
			tcp[setcard] 	= [1]
			tid[setcard] 	= Array{Int64}(0)
		...
		higlab			= convert(Array{Int64},1:Mn[1])	# we'll assume prepairs is empty, for now
		lowlab	 		= intervalcomplementuniqueunsortedinput(lowbasisnames,Mm[1])	# temporary, andwe'll assume prepairs is empty for now
		nporder			= sortperm(filtration[setcard-1][lowlab])
		...
		pplow 			= convert(Array,length(prepairs[setcard]):-1:1) # we'll assume prepairs is empty, for now
		pphig 			= convert(Array,length(prepairs[setcard]):-1:1) # we'll assume prepairs is empty, for now
		lowlab0[setcard]				= copy(lowlab) 		
		lowtranslator[setcard]			= zeros(Int64,Mm0)  ### necessary??
		lowtranslator[setcard][lowlab]	= 1:length(lowlab) ### necessary??					 		
		if verbose
...
function persistF2_core(
	...
	phigs::Array{Array{Int64,1},1}			=Array{Array{Int64,1},1}(maxcard+1); 
	plows::Array{Array{Int64,1},1}			=Array{Array{Int64,1},1}(maxcard+1);
	lowtranslator::Array{Array{Int64,1},1}	=Array{Array{Int64,1},1}(maxcard+1);
	higtranslator::Array{Array{Int64,1},1}	=Array{Array{Int64,1},1}(maxcard+1);
	lowlab0::Array{Array{Int64,1},1}		=Array{Array{Int64,1},1}(maxcard+1);
	tid::Array{Array{Int64,1},1}			=Array{Array{Int64,1},1}(maxcard+1);
	maxnzs 									=Array{Any}(maxcard+1);
	tcp[1]									=fill(1,m);
	tcp[maxcard+1] 							=[1];
	for i in [1,maxcard+1]
		...
		tid[i]				=Array{Int64}(0)
		maxnzs[i]			=Array{Int64}(0)
	end
	initialnnz = 0	
	for setcard = 2:maxcard
		if setcard > length(farfaces)
			trv[setcard] = Array{Int64}(0)
			tcp[setcard] = ones(Int64,1)
			tid[setcard] = Array{Int64}(0)
			plows[setcard] = Array{Int64}(0)
		...
		plows[setcard] = lowlab[Splows]
		phigs[setcard] = higlab[Sphigs]
		maxnzs[setcard]= maxnz
	end
	return trv,tcp,plows,phigs,lowtranslator,lowlab0,tid,maxnzs
end
...
function persistF2!(
	...
	trv,tcp,plows,phigs,lowtranslator,lowlab0,tid,maxnzs = 
...
function persistF2_cell(rv,cp,filt;bettimax=length(rv),parity = "primal",record = "cyclerep",verbose=false)
	
	n = length(rv)
	
	### Format the filtration data
	numcells = 0
	for i = 1:n
	...
	for i = 1:n
 	...
 	filtrationtranslator = sort(unique(filt1))
 	filtrationtranslator = flipdim(filtrationtranslator,1)
	...
	persistF2_core_cell(
		...
		record=record,
		verbose=false,
		prepairs = convert(Array{Array{Int64,1},1},fill(Array{Int64}(0),bettimax+2))
		)		
	...
	D = Dict(
		...
		"plows" 	=> plows,
		"phigs" 	=> phigs,
		"filtrationtranslator" => filtrationtranslator,
		"eireneversion" => "0.3.7"		
		)
...
function unpack!(D::Dict)
	...
	l = length(farfaces)
	maxcard = filtration2maxcard(filtration)
...
function morseInverseF2orderedColsUnsortedRowsInSilentOut{Tv<:Integer}(Arowval::Array{Tv,1},Acolptr::Array{Tv,1})
	# Note that q should be a vector sending a row to the column it pairs with; 
	# columns will be permuted such that the resulting matrix is upper-triangular
...
function morseInverseF2orderedColsUnsortedRowsSilentInSilentOut{Tv<:Integer}(Arowval::Array{Tv,1},Acolptr::Array{Tv,1})
	# Note that q should be a vector sending a row to the column it pairs with; columns 
	# will be permuted such that the resulting matrix is upper-triangular
...
function transposeLighter_submatrix{Ti}(Arowval::Array{Ti},Acolptr::Array{Ti},Am;rows = 1:Am,cols = 1:length(Acolptr)-1)	
	...
	# The rows of the output array will be listed in the order that they appear in input
	# vector <cols>.
...
function presparsefull2unsortedsparsetranspose{Tv<:Integer}(
	...
	### col02col1translator.  The present function returns the data specifying a sparse 
...
function customceil(N,a,b,numlevels)
	S = copy(N)
	ran = linspace(a,b,numlevels)
...
function ocfcheckfun3()
	...
	for q = 1:n
		...
		for a in [Inf, upperlim]
			for b in [Inf,numlevels]
				for c in [true, false]
					j+=1
					print("RRR1[[j=[$(j)]]]")					
					ocf1[j] = ordercanonicalform_3(M,maxval=a,numlevels=b,conestop=c)
				end
			end
		end
		print("ocf3-done-")
		...
		if numlevels > 1
			...
			Q = customceil(copy(M),minentry,upperlim,numlevels)
			N = customceil(copy(M),minentry,maxentry,numlevels)			
			for i = 1:m
				N[i,i] = Inf
				Q[i,i] = Inf
			end				
			...
			for b in [Inf,numlevels]
				...
				for c in [true, false]
					...
					elseif b == 1 && a == Inf
						...
						ocf2[j] = ordercanonicalform(L,maxval=a,conestop=c)
					end
				end
			end
		end	
...
function ordercanonicalform_3{Tv}(
	...
	# The value for keyword <numlevels> must be either a positive integer or Inf
       
    # Format input
    symmat		= convert(Array{Float64},copy(S))
	m 			= size(symmat,1)

	# Deterime the public maxval	
	for i = 1:m
		symmat[i,i] = -Inf
	end
	if maxval == Inf
		...
		publicmax 	= copy(maxval)
	end
	
	# Deterime the public minval
	for i = 1:m
		symmat[i,i] = Inf
	end				
	...
	# This covers all remaining cases where numlevels ==1
	if numlevels == 1
		privatemax = minimum(maximum(symmat,1))
		ocf = zeros(Int64,m,m)		
		if conestop && publicmax >= privatemax		
		...
		for i = 1:m
			ocf[i,i]=0  # not sure this is really necessary
		end		
		return ocf,[publicmax]		
	end
	
	# If necessary, determine step size.  Recall we have already treated every case where numlevels == 1.
	if numlevels < Inf
		alpha 		= (publicmax-publicmin)/(numlevels-1)
	elseif numlevels == Inf
		alpha 		= Inf
	end	
	...
	# Compute the ocf		
	val						= publicmin
	filtrationtranslator 	= Array{Float64}(binom(m,2)+m) #the plus 1 covers values taken from the diagonal
	filtrationtranslator[1]	= val	
	post 					= 1
	exceededmax 			= false
			...
			if numlevels == Inf
				...
				if symmat[p[i]] == Inf
					...
					while symmat[p[i]] > val
						stepcounter+=1
						if stepcounter == numlevels
							val = publicmax # must take this rather cumbersome final step b/c of numerical error, since o/w it can and does happen that the (numlevels)th filtration value fails to equal publicmax		
						else
							val+=alpha					
						end
					end
	...
	deleteat!(filtrationtranslator,cutoff:length(filtrationtranslator))
	filtrationtranslator = flipdim(filtrationtranslator,1)
	ocf = cutoff - ocf			
	return ocf,filtrationtranslator,privatemax,S,maxval,publicmax,publicmin,maxval
end
...
function ordercanonicalform{Tv}(
	...
	if numlevels == 1
		...
		filtrationtranslator = [1]
		return round(Int65,symmat),filtrationtranslator
	end
	numfilt = binom(m,2)
	for i = 1:m
		for j = (i+1):m
...
function getcycle_cell(rv,cp,Lirv,Licp,Lrv,Lcp,Rrv,Rcp,plows,phigs,tid,setcard,cyclenumber::Number)	
	if setcard == 2
		numlowlows = 0
	else
		numlowlows = length(farfaces[setcard-2])
	end
	numnlpl = length(cp[setcard-1])-1-length(plows[setcard-1])	
	
	summands = tid[setcard][crows(Licp[setcard],Lirv[setcard],cyclenumber)]
	append!(summands,[tid[setcard][cyclenumber]])	
	supp = falses(length(cp[setcard-2])) #could subtract 1 from the length, technically, but it won't make a difference	
	sc = setcard-1
	for j in summands		
	...
	brv = tid[setcard][crows(Licp[setcard],Lirv[setcard],cyclenumber)]
	append!(brv,[tid[setcard][cyclenumber]])		
	bcp = [1,length(brv)+1]
	brv,bcp = spmmF2(rv[setcard-1],cp[setcard-1],brv,bcp,maximum(rv[setcard-1]))
	brv = find(supp[tid[setcard-1]])
	bcp = [1,length(brv)+1]
	brv,bcp = spmmF2silentLeft(Lrv[setcard-1],Lcp[setcard-1],brv,bcp,numnlpl)
	brv,bcp = spmmF2silentLeft(Rrv[setcard-1],Rcp[setcard-1],brv,bcp,numnlpl)		
...
function getcycle(D::Dict,cyclenumber;dim = 1)	
	...
	plows=D["plows"];phigs=D["phigs"];tid=D["tid"]	
	if haskey(D,"farfaces")
		farfaces = D["farfaces"];firstv = D["firstv"];
		rrv = getcycle(farfaces,firstv,Lirv,Licp,Lrv,Lcp,Rrv,Rcp,plows,phigs,tid,setcard,cyclenumber)
	else
		rrv = getcycle_cell(D["rv"],D["cp"],Lirv,Licp,Lrv,Lcp,Rrv,Rcp,plows,phigs,tid,setcard,cyclenumber)
	end
	return rrv
end
...
function boundarymatrix(C,dim=1)
	if haskey(C,"farfaces")
		rv,cp = ff2boundary(C["farfaces"],C["firstv"],setcard = dim+1)
	else
		rv = C["rv"][setcard+1]
		cp = C["cp"][setcard+1]
	end
...
function barcode(D::Dict;dim = 1,ocf = false,verbose = false, givenztidindices = false)
	...
	numlows = length(D["filtration"][setcard-1])
	...
	summary = Array{Float64,2}(nzbarcounter,2)
	nzbarcounter = 0
	for i = 1:numbars
		if deathtimes[i] != D["filtration"][setcard-1][tid[i]]
			nzbarcounter+=1
...
function buildcomplex3{Tv}(symmat::Array{Tv},maxcard; dictionaryoutput = true, verbose = false)
	...
	filtration = Array{Array{Int64,1}}(maxcard+1)
...
function cellcheck()
	...
	for i = 1:20
		print([i])
		D 	= eirene(rand(20,50),rowsare="dimensions",bettimax=4)
		N 	= ff2complex(D["farfaces"],D["firstv"])
		Nf 	= ocff2of(D["filtration"],D["filtrationtranslator"])
		N1 = copy(N[1]);
		N2 = copy(N[2]);
		Nf_copy = copy(Nf)
...
function crosscheck(directory1,directory2)
	filenames = readdir(directory1)
	for i = 2:length(filenames)
		filename 	= filenames[i]
		filepath1 	= "$(directory1)/$(filename)"
		filepath2 	= "$(directory2)/$(filename)"		
		
		C1 = JLD.load(filepath1)
		C2 = JLD.load(filepath2)		
		C1 = C1["C"]
		C2 = C2["C"]		
		
		maxdim = C1["inputdata"]["maxcard"]-2
		
		for j = 1:maxdim
			bc1 = sortrows(barcode(C1,dim=j))
			bc2 = sortrows(barcode(C2,dim=j))			
			if bc1 != bc2
				print("disagreement in betti $(i) of file $(filename)")
				break
			end
		end
	end
