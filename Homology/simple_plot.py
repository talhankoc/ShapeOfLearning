...
path = '/Users/tkoc/Code/ShapeOfLearning/Homology/FloydVRLayer2/'
...
def generate_dataframes(layer_size):
	...
	lists = [h0_totals, h1_totals, h2_totals,avg_h0_lifes, avg_h1_lifes, avg_h2_lifes]
	for epoch in range(1, epochs+1):
		...
		with open(data_path, 'rb') as f:
			...
			for val, lst in zip(values, lists):
				lst.append(val)
	total_count=pd.DataFrame({'Epochs': range(1,51),  'Betti 0':h0_totals,'Betti 1':h1_totals, 'Betti 2':h2_totals})
	average_life=pd.DataFrame({'Epochs': range(1,51),  'Betti 0':avg_h0_lifes,'Betti 1':avg_h1_lifes, 'Betti 2':avg_h2_lifes})
