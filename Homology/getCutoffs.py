...
directory = "overfitMatrices/"
bdirectory = "bettiCurvesOverfit/"
...
def computeBetti(filename,cutoff_step):
	for j in range(10,101):
		...
		nums = []
		for i in [x/1000.0 for x in range(50,1000,int(cutoff_step*1000))]:
			(betti0,betti1)= stage3.main([directory+filename+str(i).replace(".","")+str(j)+".npy"])
			print("Iteration: "+str(j)+", cutoff: "+str(i)+". Betti 0:"+str(betti0)+", Betti 1:" +str(betti1))
