	...
	for epoch in range(1,epochs+1):
		...
		with open(fn, 'rb') as f:
			...
			for val, lst in zip(values, lists):
				lst.append(val)
	total_count=pd.DataFrame({'Epochs': range(1,epochs+1),  'Betti 0':h0_totals,'Betti 1':h1_totals})
	average_life=pd.DataFrame({'Epochs': range(1,epochs+1),  'Betti 0':avg_h0_lifes,'Betti 1':avg_h1_lifes})
